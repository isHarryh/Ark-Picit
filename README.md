...
4. **Win32 截图纪律**：Win32 以非管理员模式运行程序时，后台模式下 `PrintWindow` 可能会返回全黑画面，会自动回退到 `BitBlt` 截屏。连接 Windows 窗口前会检测程序是否以管理员运行（`IsUserAnAdmin`），非管理员时弹出警告并提供「重启应用」按钮，经 UAC 提权后重启，用户取消则放弃连接。
...
| :---------------------------- | :----------- | :--------------------------------------------- | :--------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------- |
| `GET /api/meta/handshake`     | 1            | 可选 `X-Client-Token`                          | 可选 `is_start=1`                                                                                                | `{version, token}`；未传或无效 token 时签发新 token；`is_start=1` 时记录访问 |
